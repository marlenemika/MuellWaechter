...
class DataModel: NSObject, ObservableObject {
    ...
    var modelId = UserDefaults.standard.integer(forKey: "modelId")
    ...
    func handleCameraPreviews(useCase: Int) async {
        ...
        for await image in imageStream {
            Task { @MainActor in
                ...
                let detections = self.detection.detectAndProcess(image: image, useCase: useCase)
                adjustObservationsInfo(observations: detections)
                let annotatedImage = labeling.labelImage(image: UIImage(ciImage: image), detections: detections)
    ...
    private func unpackPhoto(_ photo: AVCapturePhoto) -> PhotoData? {
        ...
        let observations = self.detection.detectAndProcess(image: image, useCase: -1)
        let annotatedImage = labeling.labelImage(image: UIImage(ciImage: image), detections: observations)
